      ...
      <nav className="flex items-center space-x-2 text-xs font-semibold text-slate-500 dark:text-slate-400" aria-label="Breadcrumb">
        {breadcrumbs.map((crumb, idx) => (
          <React.Fragment key={crumb.label}>
            ...
            <Link
              to={crumb.path}
              className={`flex items-center gap-1.5 hover:text-brand-500 dark:hover:text-white transition-colors duration-150 ${idx === breadcrumbs.length - 1 ? 'text-slate-850 dark:text-slate-100 font-bold' : ''
                }`}
            >
